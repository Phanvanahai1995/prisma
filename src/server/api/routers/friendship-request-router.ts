...
export const friendshipRequestRouter = router({
  send: procedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (existingRequest) {
        // Cập nhật lại yêu cầu kết bạn đã bị từ chối trước đó thành 'requested'
        await ctx.db
        ...
        // Nếu không có bản ghi trước đó, tạo yêu cầu kết bạn mới
        await ctx.db
  ...
  accept: procedure
    ...
    .mutation(async ({ ctx, input }) => {
      await ctx.db.transaction().execute(async (t) => {
        // Cập nhật trạng thái yêu cầu kết bạn hiện tại thành 'accepted'
        await t
          .updateTable('friendships')
        ...
        // Tạo bản ghi mới với người dùng hiện tại là friendUserId
        await t
          .insertInto('friendships')
          .values({
            userId: ctx.session.userId,
